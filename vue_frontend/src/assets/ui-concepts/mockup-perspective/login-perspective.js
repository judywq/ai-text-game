(function () {
  ...
  var BASE_HEIGHT = 941;
  var PLANE_WIDTH = 520;
  var PLANE_HEIGHT = 650;
  var initialDevicePixelRatio = window.devicePixelRatio || 1;

  // Calibrated live writing plane, clockwise from top-left.
  var target = [
    [294, 124],
    [808, 94],
    [825, 750],
  ...
  function solve(matrix) {
    ...
    for (var column = 0; column < rows; column += 1) {
      ...
      if (Math.abs(divisor) < 1e-10) throw new Error("Login perspective plane could not be solved.");
  ...
  function sizeScene() {
    ...
    var stage = document.getElementById("loginSceneStage");
  ...
  function init() {
    var plane = document.getElementById("loginPlane");
